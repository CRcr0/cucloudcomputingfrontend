      ...
      min-width="25">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="danger"
            @click="fnDelAdmin(scope.row.id)">delete</el-button>
        </template>
      </el-table-column>
   </el-table>
   
   <el-dialog title="Edit Admin Info" :visible.sync="pop_show" append-to-body width="95%">
...
export default {
  ...
  data () {
    var validateUserName = (rule, value, callback) => {
        if (value === '') {
          callback(new Error('Could not be Null'));
        } else {
          var reUser = /^\w{5,20}$/;
          if(reUser.test(value))
          {
            callback()
             ...
             callback(new Error('Username should be 5 to 20'));
          }
        }
    }; 
        ...
        if (value === '') {
          ...
          if(rePass.test(value))
          {
            callback()
             ...
             callback(new Error('8 to 20 with !@#$%^&*'));
          }
        }
    };
